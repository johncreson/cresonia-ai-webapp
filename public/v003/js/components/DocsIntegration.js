// DocsIntegration.js - Handles UI interactions for Google Docs integration
class DocsIntegration {
    constructor() {
        ...
        console.log('DocsIntegration component initialized');
    }
    ...
    setupEventListeners() {
        ...
        if (this.exportButton) {
            this.exportButton.addEventListener('click', async () => {
                ...
                const isAuthenticated = await window.GoogleDocsService.isAuthenticated();
                if (!isAuthenticated) {
                ...
                try {
                    await window.GoogleDocsService.exportCurrentProse();
                } catch (error) {
        ...
        // Add event listeners for document items in the sidebar
        this.setupDocItemListeners();
    }
    
    setupDocItemListeners() {
        document.addEventListener('click', (e) => {
            // Check if clicked element is a document item or its child
            const docItem = e.target.closest('.doc-item');
            if (docItem) {
                const docId = docItem.dataset.id;
                const docUrl = docItem.dataset.url;
                if (docUrl) {
    ...
    async importSelectedDocument() {
        ...
        try {
            ...
            if (this.importDocsList) {
                this.importDocsList.innerHTML += `<div class="empty-placeholder error">Error importing document: ${error.message}</div>`;
            }
        } finally {
            // Reset button state
            if (this.importSelectedDocsBtn) {
